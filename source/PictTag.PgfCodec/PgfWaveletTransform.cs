...
internal sealed class PgfWaveletTransform
{
    ...
    public PgfWaveletTransform(int width, int height, int levels, short[]? data = null)
    {
        levelCount = levels + 1;
    ...
    public PgfCodecError ForwardTransform(int level, int quant)
    {
        ...
        if (height >= PgfConstants.FilterSize)
        {
            ...
            {
                src[row1 + k] = unchecked((short)(src[row1 + k] - ((src[row0 + k] + src[row2 + k] + C1) >> 1))); // high pass
                src[row0 + k] = unchecked((short)(src[row0 + k] + ((src[row1 + k] + C1) >> 1))); // low pass
            }
            ...
            for (int i = 3; i < height - 1; i += 2)
            {
                ...
                for (int k = 0; k < width; k++)
                {
                    src[row2 + k] = unchecked((short)(src[row2 + k] - ((src[row1 + k] + src[row3 + k] + C1) >> 1))); // high pass
                    src[row1 + k] = unchecked((short)(src[row1 + k] + ((src[row0 + k] + src[row2 + k] + C2) >> 2))); // low pass
                }
            ...
            if ((height & 1) != 0)
            {
                for (int k = 0; k < width; k++)
                {
                    src[row1 + k] = unchecked((short)(src[row1 + k] + ((src[row0 + k] + C1) >> 1))); // low pass
                }
            ...
            else
            {
                ...
                for (int k = 0; k < width; k++)
                {
                    src[row2 + k] = unchecked((short)(src[row2 + k] - src[row1 + k])); // high pass
                    src[row1 + k] = unchecked((short)(src[row1 + k] + ((src[row0 + k] + src[row2 + k] + C2) >> 2))); // low pass
                }
    ...
    private static void ForwardRow(Span<short> src)
    {
        ...
        src[1] = unchecked((short)(src[1] - ((src[0] + src[2] + C1) >> 1))); // high pass
        src[0] = unchecked((short)(src[0] + ((src[1] + C1) >> 1))); // low pass

        for (; i < width - 1; i += 2)
        {
            src[i] = unchecked((short)(src[i] - ((src[i - 1] + src[i + 1] + C1) >> 1))); // high pass
            src[i - 1] = unchecked((short)(src[i - 1] + ((src[i - 2] + src[i] + C2) >> 2))); // low pass
        }

        if ((width & 1) != 0)
        {
            src[i - 1] = unchecked((short)(src[i - 1] + ((src[i - 2] + C1) >> 1))); // low pass
        }
        else
        {
            src[i] = unchecked((short)(src[i] - src[i - 1])); // high pass
            src[i - 1] = unchecked((short)(src[i - 1] + ((src[i - 2] + src[i] + C2) >> 2))); // low pass
        }
    }
    ...
    private void InterleavedToSubbands(int destLevel, ReadOnlySpan<short> loRow, ReadOnlySpan<short> hiRow)
    {
    ...
    public PgfCodecError InverseTransform(int srcLevel, out int width, out int height, out short[] data)
    {
        ...
        short[] destBuffer = destBand.GetBuffer();
        int origin = 0;
        ...
        if (destHeight >= PgfConstants.FilterSize)
        {
            ...
            for (int k = 0; k < width; k++)
            {
                destBuffer[row0 + k] = unchecked((short)(destBuffer[row0 + k] - ((destBuffer[row1 + k] + C1) >> 1))); // even
            }
            ...
            for (int i = 2; i < destHeight - 1; i += 2)
            {
                ...
                for (int k = 0; k < width; k++)
                {
                    destBuffer[row2 + k] = unchecked((short)(destBuffer[row2 + k] - ((destBuffer[row1 + k] + destBuffer[row3 + k] + C2) >> 2))); // even
                    destBuffer[row1 + k] = unchecked((short)(destBuffer[row1 + k] + ((destBuffer[row0 + k] + destBuffer[row2 + k] + C1) >> 1))); // odd
                }
            ...
            if ((destHeight & 1) != 0)
            {
                ...
                for (int k = 0; k < width; k++)
                {
                    destBuffer[row2 + k] = unchecked((short)(destBuffer[row2 + k] - ((destBuffer[row1 + k] + C1) >> 1))); // even
                    destBuffer[row1 + k] = unchecked((short)(destBuffer[row1 + k] + ((destBuffer[row0 + k] + destBuffer[row2 + k] + C1) >> 1))); // odd
                }
            ...
            else
            {
                for (int k = 0; k < width; k++)
                {
                    destBuffer[row1 + k] = unchecked((short)(destBuffer[row1 + k] + destBuffer[row0 + k]));
                }
    ...
    private static void InverseRow(Span<short> dest)
    {
        ...
        dest[0] = unchecked((short)(dest[0] - ((dest[1] + C1) >> 1))); // even

        for (; i < width - 1; i += 2)
        {
            dest[i] = unchecked((short)(dest[i] - ((dest[i - 1] + dest[i + 1] + C2) >> 2))); // even
            dest[i - 1] = unchecked((short)(dest[i - 1] + ((dest[i - 2] + dest[i] + C1) >> 1))); // odd
        }

        if ((width & 1) != 0)
        {
            dest[i] = unchecked((short)(dest[i] - ((dest[i - 1] + C1) >> 1))); // even
            dest[i - 1] = unchecked((short)(dest[i - 1] + ((dest[i - 2] + dest[i] + C1) >> 1))); // odd
        }
        else
        {
            dest[i - 1] = unchecked((short)(dest[i - 1] + dest[i - 2])); // odd
        }
    }
    ...
    private void SubbandsToInterleaved(int srcLevel, Span<short> loRow, Span<short> hiRow)
    {
        int width = loRow.Length;
        int wquot = width >> 1;
        bool wrem = (width & 1) != 0;
